...
A empresa **CloudTech Solutions** enfrentou recentemente um incidente crítico em uma de suas unidades operacionais remotas: o sistema de climatização falhou, causando um superaquecimento e o desligamento automático do servidor principal (*thermal shutdown*). Isso resultou em 4 horas de *downtime* (inatividade) e severos prejuízos financeiros.
...
3. **Camada de Nuvem (Cloud Computing):**
   ...
   - **Dashboard UI:** Interface moderna em *Dark Mode* para controle visual, geração de gráficos históricos (24h) e controle manual remoto do exaustor.
...
## 📋 6. Preparação para o Pitch e Defesa Técnica

Como critério de avaliação da disciplina, os grupos devem estar preparados para demonstrar e defender os seguintes pontos:
* **Conectividade & Integridade:** Comprovar a submissão correta do JSON para a nuvem em < 10 segundos de inicialização.
* **Resiliência de Comando:** Testar a ativação do exaustor através do botão virtual do painel com resposta instantânea no simulador.
* **Autonomia e Sobrevivência (Edge Continuity):** Demonstrar que aumentando a temperatura do sensor para mais de 28°C no Wokwi, o sistema força a ativação sem depender dos botões da Nuvem.

---
*Desenvolvido como Trabalho Final para a disciplina de Cloud Computing e Internet das Coisas (IoT).*
